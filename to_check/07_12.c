...
void deal_five(const int wDeck[][13], const char *wFace[],
	       const char *wSuit[], int wPlayer[][2]);
...
int testHand(int const wPlayer[][2])
{
    ...
    if (wPlayer[0][0] == (wPlayer[1][0] - 1)
	&& wPlayer[1][0] == (wPlayer[2][0] - 1)
	&& wPlayer[2][0] == (wPlayer[3][0] - 1)
	&& wPlayer[3][0] == (wPlayer[4][0] - 1)) {
	printf("Straight\n");
	return 1;
    }
    // then for a flush if column1 of the matrix of cards of the player hold all the same value
    else if (wPlayer[0][1] == wPlayer[1][1]
	     && wPlayer[1][1] == wPlayer[2][1]
	     && wPlayer[2][1] == wPlayer[3][1]
	     && wPlayer[3][1] == wPlayer[4][1]) {
	printf("Flush\n");
	return 2;
    }
